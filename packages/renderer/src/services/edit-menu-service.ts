...
export class EditMenuService {
   ...
   */
  async search(term: string, options: { matchCase?: boolean; useRegex?: boolean } = {}): Promise<SearchResult[]> {
    try {
   ...
   */
  findNext(): boolean {
    if (this.searchState.searchResults.length === 0) return false;
   ...
   */
  async replaceOne(replacementText: string): Promise<boolean> {
    try {
   ...
   */
  async replaceAll(replacementText: string): Promise<number> {
    try {
      let replacedCount = 0;
  ...
  private setupClipboardListeners(): void {
  ...
  private async insertText(element: HTMLElement, text: string): Promise<void> {
    if (element.tagName === 'INPUT' || element.tagName === 'TEXTAREA') {
      const input = element as HTMLInputElement;
  ...
  private findMatches(content: string, term: string, options: { matchCase?: boolean; useRegex?: boolean }): Array<{ text: string; start: number; end: number }> {
  ...
  private getContext(content: string, start: number, end: number, contextLength: number = 50): string {
  ...
  private highlightSearchResult(result: SearchResult): void {
  ...
  private async replaceTextInElement(element: HTMLElement, start: number, end: number, replacement: string): Promise<boolean> {
    try {
      if (element.tagName === 'INPUT' || element.tagName === 'TEXTAREA') {
        const input = element as HTMLInputElement;
        const before = input.value.substring(0, start);
  ...
  private recordAction(action: Omit<UndoRedoAction, 'id' | 'timestamp'>): void {
  ...
  private async applyUndoRedoAction(action: UndoRedoAction, direction: 'undo' | 'redo'): Promise<void> {
...
export const editMenuService = EditMenuService.getInstance(); 