...
export class WorkflowRefinementAgent extends BaseLLMAgent {
  constructor() {
   ...
   */
  async refineWorkflow(request: WorkflowRefinementRequest): Promise<WorkflowRefinementResponse> {
   ...
   */
  private formatRefinementRequest(request: WorkflowRefinementRequest): string {
...
Please respond with the updated workflow in the specified JSON format.`;
   ...
   */
  private parseRefinementResponse(response: LLMResponse): WorkflowRefinementResponse {
    try {
   ...
   */
  private generateUpdatedVisualization(): string {
    return `┌─────────────────────────────────────────┐
│           WORKFLOW START                │
│           User Request Entry             │
└─────────────┬───────────────────────────┘
...
└─────────────────────────────────────────┘`;
   ...
   */
  private generateUpdatedWorkflow(): Partial<WorkflowTemplate> {
    return {
  ...
  analyzeRefinementComplexity(userRequest: string): {
    ...
    if (request.includes('step')) affectedComponents.push('steps');
    if (request.includes('parameter')) affectedComponents.push('parameters');
    if (request.includes('tool')) affectedComponents.push('toolCalls');
    if (request.includes('flow') || request.includes('sequence')) affectedComponents.push('flow');
    if (request.includes('error') || request.includes('handling')) affectedComponents.push('errorHandling');
