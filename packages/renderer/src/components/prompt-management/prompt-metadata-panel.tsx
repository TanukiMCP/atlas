...
export const PromptMetadataPanel: React.FC = () => {
  const { selectedPrompt } = usePromptStore();
  const [activeTab, setActiveTab] = useState<'info' | 'variables' | 'guidance' | 'default'>('info');
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className={`inline-flex px-2 py-1 rounded text-xs font-medium ${
                  selectedPrompt.complexity === 'low' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' :
              ...
              <div className="flex flex-wrap gap-1">
                {selectedPrompt.tags.map(tag => (
                  <span
                    key={tag}
                    className="inline-flex px-2 py-1 bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-400 text-xs rounded"
                  >
                    {tag}
                  </span>
                ))}
