...
export const MentalModelSchema = z.object({
  modelName: z.nativeEnum(MentalModel).describe('The mental model to apply'),
  problem: z.string().describe('The problem or situation to analyze'),
  reasoning: z.string().optional().describe('Additional reasoning context'),
  steps: z.array(z.string()).optional().describe('Specific analysis steps'),
  conclusion: z.string().optional().describe('Final conclusion or recommendation')
});

export class MentalModelsTool {
  private modelDefinitions: Map<MentalModel, {
    name: string;
    description: string;
    framework: string[];
   ...
   */
  public async execute(params: z.infer<typeof MentalModelSchema>): Promise<MentalModelAnalysis> {
   ...
   */
  public suggestModel(problemDescription: string): {
  ...
  private applyFirstPrinciples(problem: string, model: any) {
    return {
      steps: [
  ...
  private applyOpportunityCost(problem: string, model: any) {
    return {
      steps: [
  ...
  private applyErrorPropagation(problem: string, model: any) {
    return {
      steps: [
  ...
  private applyRubberDuck(problem: string, model: any) {
    return {
      steps: [
  ...
  private applyParetoPrinciple(problem: string, model: any) {
    return {
      steps: [
        `Optimizing: ${problem}`,
  ...
  private applyOccamsRazor(problem: string, model: any) {
    return {
      steps: [
