...
const WorkflowGenerationDialog = ({ isOpen, onClose, chatTranscript, onWorkflowSaved }) => {
    ...
    return (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg w-[90vw] h-[85vh] flex flex-col">
        ...
        {/* Progress Bar */}
        {state.loading && (<div className="px-4 py-2 border-b border-gray-200 dark:border-gray-700">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 flex flex-col border-r border-gray-200 dark:border-gray-700">
            <div className="p-4 border-b border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <button onClick={() => setIsDirectEditMode(!isDirectEditMode)} className={`px-3 py-1 text-sm rounded-lg transition-colors ${isDirectEditMode
            ...
            <div className="flex-1 p-4">
              {state.currentStep === 'analyzing' || state.currentStep === 'synthesizing' ? (<div className="h-full flex items-center justify-center">
                  ...
                  {isDirectEditMode ? (<textarea ref={visualizationTextareaRef} value={state.workflowVisualization} onChange={(e) => handleVisualizationChange(e.target.value)} className="w-full h-full font-mono text-sm p-4 border border-gray-300 dark:border-gray-600 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="ASCII workflow visualization will appear here..."/>) : (<div className="h-full overflow-auto">
          ...
          <div className="w-80 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {state.refinementHistory.length === 0 ? (<div className="text-center text-gray-500 dark:text-gray-400 text-sm">
                  <p>No refinements yet.</p>
                  <p>Type your refinement requests below.</p>
                </div>) : (state.refinementHistory.map((item) => (<div key={item.id} className="space-y-2">
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
                      ...
                      {item.changesSummary && (<div className="text-xs text-gray-600 dark:text-gray-400 mt-2 italic">
            ...
            <div className="p-4 border-t border-gray-200 dark:border-gray-700">
              <div className="flex flex-col space-y-2">
                <textarea value={refinementInput} onChange={(e) => setRefinementInput(e.target.value)} placeholder="Type refinement requests here..." className="w-full h-20 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-blue-500" disabled={state.loading}/>
                <button onClick={handleRefinementSubmit} disabled={!refinementInput.trim() || state.loading} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
                  Send
                </button>
              </div>
            </div>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <div className="flex space-x-2">
              <button onClick={handleRegenerate} disabled={state.loading} className="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 disabled:opacity-50">
                🔄 Regenerate
              </button>
            </div>
            
            <div className="flex space-x-2">
              <button onClick={handleClose} className="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600">
                Cancel
              </button>
              <button onClick={handleSaveWorkflow} disabled={state.loading || !state.workflowVisualization} className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed">
                💾 Save Workflow
              </button>
            </div>
          </div>
