...
const PromptEditorPanel = () => {
    ...
    return (<div className="h-full flex flex-col bg-white dark:bg-gray-900">
      ...
      <div className="flex-shrink-0 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            {selectedPrompt.isModified && (<div className="px-2 py-1 bg-yellow-100 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-300 text-xs rounded">
                Modified
              </div>)}
          </div>
          ...
          <div className="flex items-center space-x-2">
            <button onClick={handleValidate} className="px-3 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded hover:bg-gray-50 dark:hover:bg-gray-700">
              Validate
            </button>
            <button onClick={handleExport} className="px-3 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded hover:bg-gray-50 dark:hover:bg-gray-700">
              Export
            </button>
            {selectedPrompt.isModified && (<button onClick={handleReset} className="px-3 py-1 text-sm border border-gray-300 dark:border-gray-600 text-red-600 dark:text-red-400 rounded hover:bg-red-50 dark:hover:bg-red-900/20">
                Reset
              </button>)}
            <button onClick={handleSave} disabled={!hasUnsavedChanges || isLoading} className="px-3 py-1 text-sm bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        {/* Validation Results */}
        {showValidation && validation && (<div className="mt-3">
            {validation.errors.length > 0 && (<div className="mb-2 p-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded">
                <div className="text-sm font-medium text-red-700 dark:text-red-300 mb-1">
                  Validation Errors:
                </div>
                {validation.errors.map((error, index) => (<div key={index} className="text-sm text-red-600 dark:text-red-400">
                    • {error.message}
                  </div>))}
              </div>)}
            
            {validation.warnings.length > 0 && (<div className="mb-2 p-2 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded">
                <div className="text-sm font-medium text-yellow-700 dark:text-yellow-300 mb-1">
                  Validation Warnings:
                </div>
                {validation.warnings.map((warning, index) => (<div key={index} className="text-sm text-yellow-600 dark:text-yellow-400">
                    • {warning.message}
                  </div>))}
              </div>)}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <textarea ref={textareaRef} value={currentContent} onChange={(e) => handleContentChange(e.target.value)} placeholder="Enter prompt content..." className="flex-1 w-full p-4 border-none resize-none focus:outline-none 
                     bg-white dark:bg-gray-900 text-gray-900 dark:text-white
                     font-mono text-sm leading-relaxed" style={{ minHeight: '400px' }}/>
      </div>
      ...
      <div className="flex-shrink-0 bg-gray-50 dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-2">
        <div className="flex items-center justify-between text-xs text-gray-600 dark:text-gray-400">
          ...
          <div className="flex items-center space-x-2">
            ...
            {hasUnsavedChanges && (<span className="text-yellow-600 dark:text-yellow-400">• Unsaved changes</span>)}
          </div>
        </div>
      </div>
    </div>);
