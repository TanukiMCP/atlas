...
class UserPreferences {
    preferences;
    storageKey = 'tanukimcp-tool-preferences';
    constructor() {
    ...
    async updateToolWeight(toolId, weight) {
        const [sourceId] = toolId.split(':');
    ...
    async recordToolUsage(toolId, success, executionTime) {
    ...
    async addConflictResolutionRule(rule) {
    ...
    async removeConflictResolutionRule(pattern) {
    ...
    async updateCategoryVisibility(categoryId, visible) {
    ...
    async updateCategoryPriority(categoryId, priority) {
        const category = this.preferences.toolCategories.find(c => c.categoryId === categoryId);
        if (category) {
