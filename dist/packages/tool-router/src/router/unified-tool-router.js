...
exports.UnifiedToolRouter = void 0;
const eventemitter3_1 = require("eventemitter3");
const tool_discovery_1 = require("./tool-discovery");
const context_analyzer_1 = require("./context-analyzer");
const conflict_resolver_1 = require("./conflict-resolver");
const execution_router_1 = require("./execution-router");
const fuzzy_search_1 = require("../search/fuzzy-search");
const tool_indexer_1 = require("../search/tool-indexer");
const user_preferences_1 = require("../preferences/user-preferences");
const performance_monitor_1 = require("../execution/performance-monitor");
class UnifiedToolRouter extends eventemitter3_1.EventEmitter {
    toolDiscovery;
    contextAnalyzer;
    conflictResolver;
    executionRouter;
    ...
    initializeComponents() {
        this.toolDiscovery = new tool_discovery_1.ToolDiscovery();
        this.contextAnalyzer = new context_analyzer_1.ContextAnalyzer();
        this.conflictResolver = new conflict_resolver_1.ConflictResolver();
        this.executionRouter = new execution_router_1.ExecutionRouter();
        this.fuzzySearch = new fuzzy_search_1.FuzzySearch();
        this.toolIndexer = new tool_indexer_1.ToolIndexer();
        this.userPreferences = new user_preferences_1.UserPreferences();
    ...
    async searchTools(query, context, options) {
        if (!this.isInitialized) {
    ...
    async getToolsByCategory(categoryId, context) {
    ...
    async getAvailableCategories(context) {
    ...
    async processDiscoveredTools(discoveredTools) {
        const unifiedTools = [];
    ...
    calculateToolScore(tool, query, context) {
        let score = 0;
    ...
    async createRouteDecision(tool, context) {
        const alternatives = [];
        const reasoning = [];
    ...
    async handleExecutionError(tool, parameters, context, error) {
    ...
    async handleToolsDiscovered(tools) {
        console.log(`Discovered ${tools.length} tools from sources`);
        await this.refreshToolCatalog();
    }
    handleSourceUnavailable(sourceId) {
...
exports.UnifiedToolRouter = UnifiedToolRouter;
//# sourceMappingURL=unified-tool-router.js.map