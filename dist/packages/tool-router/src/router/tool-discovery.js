...
exports.ToolDiscovery = void 0;
const eventemitter3_1 = require("eventemitter3");
class ToolDiscovery extends eventemitter3_1.EventEmitter {
    builtinToolsSource; // Reference to builtin MCP server
    externalSourcesHub; // Reference to external MCP hub
    ...
    async discoverAllTools() {
        ...
        try {
            ...
            this.emit('discovery:error', 'unknown', error);
        }
        finally {
    ...
    async discoverBuiltinTools() {
        ...
        try {
            ...
            this.emit('discovery:error', 'builtin', error);
        }
        return tools;
    }
        ...
        try {
            ...
            for (const server of connectedServers) {
                try {
                    ...
                    this.emit('discovery:error', server.id, serverError);
                }
            }
            ...
            this.emit('discovery:error', 'external', error);
        }
        return tools;
    }
    inferCategory(toolName, description) {
    ...
    extractTags(description, toolName) {
        const tags = [];
        const text = `${toolName} ${description}`.toLowerCase();
